...
public class Script_MidGrass : MonoBehaviour
{
    ...
    public static GameObject[] LinesGrass;         // to store lines in array

    bool[,] FenceGrid = new bool[Settings.DefaultLanes[1] + Settings.ExtraLanes[1], MaxFenceAmountPerLane];                    // to prevent fence overlapping with usage of while loop
    ...
    void Start()
    {
        LinesGrass = new GameObject[Settings.DefaultLanes[1] + Settings.ExtraLanes[1]];
    ...
    {
        for (int i = 0; i < Settings.DefaultLanes[1] + Settings.ExtraLanes[1]; i++)
        {
    ...
    void InstantiateMidGrass()
    {
        ...
        for (int i = 0; i < Settings.DefaultLanes[1] + Settings.ExtraLanes[1]; i++)
        {
            InstantiationLinePos.Set(0, (Settings.DefaultLanes[0] + Settings.ExtraLanes[0]) + i - 3, 0);              // Start counting from amount of Road Lines.
    ...
    void InstantiateSnake()
    {
        ...
        InstantiationLinePos.Set(0, (Settings.DefaultLanes[0] + Settings.ExtraLanes[0]) - 3, -1);
    ...
    void InstantiateFences()
    {
        for (int i = 0; i < Settings.DefaultLanes[1] + Settings.ExtraLanes[1]; i++)
        {
            ...
            for (int j = 0; j < FenceAmountPerLane; j++)
            {
                ...
                InstantiationPos.Set(RolledPosition - 9, (Settings.DefaultLanes[0] + Settings.ExtraLanes[0]) + i - 3 - FenceYPositionOffset, -4);
